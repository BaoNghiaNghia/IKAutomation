...
namespace IKAutomation.GameDetection.Tests
{
    internal static class Program
    {
        ...
        private static int Main()
        {
            ...
            Run("ResourceExpiryDialog has priority over TeamSelection", ResourceExpiryHasPriorityOverTeam);
        ...
        private static void ResourceExpiryHasPriorityOverTeam()
        {
            GameDetectionResult result = DetectWithMatches(
                TemplateId.ResourceExpiryDialogAnchor, TemplateId.StorageLimitCancelButton,
                TemplateId.TeamSelectionPanelAnchor, TemplateId.TeamActionButtonEnabled);
            Equal(GameState.ResourceExpiryDialog, result.State,
                "ResourceExpiryDialog must have priority over TeamSelection.");
        }
        ...
        private static void CityStableMapButtonFallback()
        {
            var matcher = new FakeImageMatcher { CityStableOnly = true };
            matcher.Matches.Add(TemplateId.CityToWorldMapButton);
            GameDetectionResult result = CreateDetector(
                new FakeLdPlayerClient(), matcher: matcher)
                .DetectAsync("IK-1", TestToken).GetAwaiter().GetResult();
            Equal(GameState.City, result.State, "Stable City map-button fallback failed.");
            GameDetectionEvidence evidence = result.Evidence.First(
                item => item.TemplateId == TemplateId.CityToWorldMapButton);
            Assert(evidence.Found && evidence.Message.IndexOf("stable icon center",
        ...
        private static TemplateId[] RequiredIds() => new[] { TemplateId.ResourceSearchPanelAnchor,
            TemplateId.ResourceExpiryDialogAnchor, TemplateId.StorageLimitDialogAnchor, TemplateId.StorageLimitCancelButton,
        ...
        private sealed class FakeImageMatcher : IImageMatcher
        {
            ...
            public ImageMatchResult Find(byte[] screenshot, byte[] template, ImageRegion? region = null)
            {
                ...
                callsByTemplate[id] = callsByTemplate.TryGetValue(id, out int calls) ? calls + 1 : 1;
                Regions[id] = region;
