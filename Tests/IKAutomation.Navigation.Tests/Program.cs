...
namespace IKAutomation.Navigation.Tests
{
    internal static class Program
    {
        ...
        private static int Main()
        {
            ...
            Run("Same-territory coordinates are the default primary strategy", SameTerritoryCoordinatesArePrimary);
            ...
            Run("Territory reposition cancels blocking dialog after Back", TerritoryRepositionCancelsBlockingDialog);
            ...
            Run("Territory reposition refreshes incomplete fast-path evidence", TerritoryRepositionRefreshesIncompleteFastPathEvidence);
        ...
        private static void TerritoryRepositionTapsMatchedAnchors() { var f=Setup(WorldMapWithPin(),ContinentMapWithAnchors(),ContinentMapWithAnchors(),State(GameState.WorldMap)); var r=f.Service.RepositionToAllianceTerritoryAsync("d",Token).GetAwaiter().GetResult(); Assert(r.Success,"Expected recovery success."); Equal(4,f.Client.TapCalls,"Tap count includes the bounded coordinate-reader availability probe."); Equal(197,f.Client.LastX,"Pin X."); Equal(37,f.Client.LastY,"Pin Y."); }
        ...
        private static void TerritoryRepositionCancelsBlockingDialog() { var f=Setup(State(GameState.ResourceSearchPanel),UnknownWithWorldMapCancel(),WorldMapWithPin(),ContinentMapWithAnchors(),ContinentMapWithAnchors(),State(GameState.WorldMap)); var r=f.Service.RepositionToAllianceTerritoryAsync("d",Token).GetAwaiter().GetResult(); Assert(r.Success,"Blocking dialog recovery failed."); Equal(1,f.Client.BackCalls,"Back count."); Equal(5,f.Client.TapCalls,"Coordinate availability probe, cancel, and navigation taps."); }
        ...
        private static void TerritoryRepositionRefreshesIncompleteFastPathEvidence() { var f=Setup(State(GameState.WorldMap,true),ContinentMapWithAnchors(),ContinentMapWithAnchors(),State(GameState.WorldMap)); f.Detector.ScreenshotStates.Enqueue(WorldMapWithPin()); var r=f.Service.RepositionToAllianceTerritoryAsync("d",Token).GetAwaiter().GetResult(); Assert(r.Success,"A bounded full-frame refresh should recover missing pin-button evidence."); Equal(1,f.Detector.FrameCalls,"Full-frame refresh count."); Equal(4,f.Client.TapCalls,"Expected refresh flow plus bounded coordinate-reader availability probe."); Assert(r.Transitions.Any(x=>x.Message.Contains("bounded full-frame evidence refresh")),"Refresh transition was not recorded."); }
