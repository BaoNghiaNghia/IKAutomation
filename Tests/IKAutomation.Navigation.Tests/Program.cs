...
namespace IKAutomation.Navigation.Tests
{
    internal static class Program
    {
        ...
        private static int Main()
        {
            ...
            Run("Same-territory reposition uses X/Y coordinate input", SameTerritoryCoordinatesArePrimary);
            Run("Resource Area Lv2 points use X/Y entry without territory color", ResourceAreaLv2UsesCoordinateEntryWithoutTerritoryColor);
        ...
        private static void SameTerritoryCoordinatesArePrimary() { string source=ReadNavigationSource(); string core=Between(source,"private async Task<NavigationResult> RepositionToAllianceTerritoryCoreAsync","private async Task<HomeLocationEvidence>"); Assert(core.Contains("SameTerritoryCoordinateInput"),"Coordinate-input strategy was not wired into the recovery core."); Assert(core.Contains("TrySameTerritoryCoordinateAsync"),"Same-territory recovery must use the X/Y coordinate workflow."); Assert(!core.Contains("TryScreenPointFallbackAsync"),"The generic same-colour workflow must not consume the Resource Area Lv2 screen-point strategy."); }
        private static void ResourceAreaLv2UsesCoordinateEntryWithoutTerritoryColor() { string coordinator=ReadResourceAreaLv2CoordinatorSource(); Assert(coordinator.Contains("OpenMapAndEnterCoordinatesAsync"),"The Lv2 toast branch must invoke the dedicated X/Y entry API."); Assert(coordinator.Contains("point.BasePoint.X") && coordinator.Contains("point.BasePoint.Y"),"The selected predefined values must be entered as map X/Y coordinates without screen scaling."); Assert(!coordinator.Contains("OpenMapAndTapPointAsync("),"The Lv2 toast branch must not tap a predefined value as a screen pixel."); Assert(!coordinator.Contains("RepositionToAllianceTerritoryAsync"),"The Lv2 toast branch must not invoke same-territory color selection."); string source=ReadNavigationSource(); string method=Between(source,"private async Task<NavigationResult> OpenMapAndEnterCoordinatesCoreAsync","private async Task<NavigationResult> OpenMapAndTapPointCoreAsync"); Assert(method.Contains("ApplyCoordinateCandidateAsync"),"The dedicated flow must reuse verified field focus, clear and input handling."); Assert(method.Contains("ContinentMapSearchTargetPinAfterResourceAreaCoordinates"),"The flow must use the destination pin after submitting X/Y, not submit the coordinate pin twice."); Assert(!method.Contains("TryClassifyPinTerritory"),"The toast branch must not classify territory color."); }
        ...
        private static string ReadResourceAreaLv2CoordinatorSource() { DirectoryInfo directory=new DirectoryInfo(AppContext.BaseDirectory); while(directory!=null) { string path=Path.Combine(directory.FullName,"ADB","Infrastructure","ResourceSearch","ResourceAreaLv2RecoveryCoordinator.cs"); if(File.Exists(path)) return File.ReadAllText(path); directory=directory.Parent; } throw new InvalidOperationException("ResourceAreaLv2RecoveryCoordinator source was not found."); }
