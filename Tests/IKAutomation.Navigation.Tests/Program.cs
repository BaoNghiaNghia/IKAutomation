...
namespace IKAutomation.Navigation.Tests
{
    internal static class Program
    {
        ...
        private static void SameTerritoryCoordinatesArePrimary() { var o=new WorldMapNavigationOptions(10,1,2,true,1,100,20,3000,1,true);var f=SetupWithOptions(o,WorldMapWithPin(),ContinentMapWithPinPair(670,390,710,350),ContinentMapWithCoordinateTargetAndAnchors(),ContinentMapWithCoordinateTargetAndAnchors(),ContinentMapWithCoordinateTargetAndAnchors(),ContinentMapWithCoordinateTargetAndAnchors(),State(GameState.WorldMap));f.Detector.ScreenshotStates.Enqueue(ContinentMapWithCoordinateTargetAndAnchors());QueueFocusedCoordinates(f,623,349,1);f.Client.Screenshots.Enqueue(SolidScreenshot(TerritoryGreen));f.Client.Screenshots.Enqueue(FallbackTerritoryScreenshot(TerritoryGreen,TerritoryGreen));var r=f.Service.RepositionToAllianceTerritoryAsync("d",Token).GetAwaiter().GetResult();Assert(r.Success,"Reposition should complete through a verified safe strategy. "+r.Message);int coordinate=r.Transitions.ToList().FindIndex(x=>x.Operation=="Strategy"&&x.Message=="SameTerritoryCoordinate");int nearby=r.Transitions.ToList().FindIndex(x=>x.Operation=="Strategy"&&x.Message=="ValidatedNearbyPin");Assert(coordinate>=0,"Primary strategy order was not reported.");Assert(nearby<0||coordinate<nearby,"Nearby-pin observation started before coordinate strategy."); }
