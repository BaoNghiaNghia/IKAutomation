...
namespace IKAutomation.Navigation.Tests
{
    internal static class Program
    {
        ...
        private static void TerritoryRepositionCancelsBlockingDialog() { var f=Setup(State(GameState.ResourceSearchPanel),UnknownWithCancel(),WorldMapWithPin(),ContinentMapWithAnchors(),ContinentMapWithAnchors(),State(GameState.WorldMap)); var r=f.Service.RepositionToAllianceTerritoryAsync("d",Token).GetAwaiter().GetResult(); Assert(r.Success,"Blocking dialog recovery failed."); Equal(1,f.Client.BackCalls,"Back count."); Equal(4,f.Client.TapCalls,"Cancel plus navigation taps."); }
