...
internal static class Program
{
    ...
    static int Main()
    {
        ...
        Run("Target-level-too-low toast during configuration switches resource", ConfigurationTargetLevelTooLowToastSwitchesResource);
        Run("Target-level-too-low search skips remaining levels", TargetLevelTooLowSearchSkipsRemainingLevels);
    ...
    static void ConfigurationTargetLevelTooLowToastSwitchesResource(){var h=new H();h.Search.Outcomes.Enqueue(ResourceSearchOutcome.ResourceNotFound);h.Config.FailLevel=6;h.Matcher.ToastFrames.Add(3);h.Matcher.MatchedToastTemplates.Clear();h.Matcher.MatchedToastTemplates.Add(TemplateId.ResourceTargetLevelTooLowToastAnchor);h.Matcher.MatchedToastTemplates.Add(TemplateId.ResourceTargetLevelSeasonMapToastAnchor);var r=Go(h);Eq(ResourceLevelFallbackOutcome.ResourceLevelsExhausted,r.Outcome,"outcome");Eq("TargetLevelTooLow",r.Attempts.Last().MatchedNotFoundVariant,"variant");Eq(1,h.Search.Calls,"search retry");}
    static void TargetLevelTooLowSearchSkipsRemainingLevels(){var h=new H();h.Search.Default=ResourceSearchOutcome.ResourceNotFound;h.Search.NotFoundVariant="TargetLevelTooLow";var r=Go(h);Eq(ResourceLevelFallbackOutcome.ResourceLevelsExhausted,r.Outcome,"outcome");Eq(1,h.Search.Calls,"search count");Is(new[]{7}.SequenceEqual(h.Config.Levels),"configured levels");Eq("TargetLevelTooLow",r.Attempts.Single().MatchedNotFoundVariant,"variant");}
    ...
    sealed class FakeSearch:IResourceSearchExecutionService{public int Calls;public ResourceSearchOutcome Default=ResourceSearchOutcome.ResourceLocated;public string NotFoundVariant="SearchOtherRegion";public Queue<ResourceSearchOutcome> Outcomes=new Queue<ResourceSearchOutcome>();public List<ResourceSearchExecutionRequest> Requests=new List<ResourceSearchExecutionRequest>();public CancellationTokenSource CancelAfterNotFound;public Task<ResourceSearchExecutionResult> ExecuteAsync(string d,ResourceSearchExecutionRequest r,CancellationToken t){t.ThrowIfCancellationRequested();Calls++;Requests.Add(r);var o=Outcomes.Count>0?Outcomes.Dequeue():Default;var x=new ResourceSearchExecutionResult{Outcome=o,Success=o==ResourceSearchOutcome.ResourceLocated,FinalState=o==ResourceSearchOutcome.ResourceLocated?GameState.ResourcePopup:GameState.ResourceSearchPanel,MatchedNotFoundVariant=o==ResourceSearchOutcome.ResourceNotFound?NotFoundVariant:null,Message=o.ToString()};if(o==ResourceSearchOutcome.ResourceNotFound)CancelAfterNotFound?.Cancel();return Task.FromResult(x);}}
