...
internal static class Program
{
    ...
    sealed class FakeConfig:IResourceSearchConfigurationService{public int Calls,FailLevel;public List<int> Levels=new List<int>();public Task<ResourceSearchConfigurationResult> ConfigureAsync(string d,ResourceSearchConfigurationRequest r,CancellationToken t){t.ThrowIfCancellationRequested();Calls++;Levels.Add(r.TargetLevel);bool ok=r.TargetLevel!=FailLevel;return Task.FromResult(new ResourceSearchConfigurationResult{Success=ok,ResourceVerified=ok,LevelVerified=ok,FilterVerified=ok,FinalState=GameState.ResourceSearchPanel,Message=ok?"configured":"failed",ErrorMessage=ok?null:"configuration"});}}
