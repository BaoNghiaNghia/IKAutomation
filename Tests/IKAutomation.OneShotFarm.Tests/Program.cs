...
internal static class Program
{
    ...
    static void AvailabilityUsesRosterRoi(){var f=new AvailabilityFixture();f.Matcher.ReadyRows.Add(4);var r=f.Service.CheckAsync("LDPlayer",default(CancellationToken)).GetAwaiter().GetResult();Is(r.Success&&r.AnyReadyTeam,"ready not found");Eq(1,f.Client.Captures,"captures");Eq(8,f.Matcher.Regions.Count,"badge and ready scans");foreach(var region in f.Matcher.Regions.Take(4))Is(region.Value.X==0&&region.Value.Y==290&&region.Value.Width==150&&region.Value.Height==280,"badge scan must use full roster");for(int i=0;i<4;i++){var region=f.Matcher.Regions[4+i].Value;int badgeCenter=310+(i*70);Is(region.X==0&&region.Y<=badgeCenter&&region.Y+region.Height>=badgeCenter&&region.Width==150&&region.Height<=64,"wrong dynamic row ROI "+i);}}
    ...
    sealed class AvailabilityMatcher:IImageMatcher{public readonly HashSet<int> PresentRows=new HashSet<int>(new[]{1,2,3,4});public readonly HashSet<int> ReadyRows=new HashSet<int>();public readonly List<ImageRegion?> Regions=new List<ImageRegion?>();public ImageMatchResult Find(byte[] s,byte[] t,ImageRegion? r=null){Regions.Add(r);if(!r.HasValue||t==null||t.Length==0)return ImageMatchResult.NotFound();TemplateId id=(TemplateId)t[0];int badgeRow=id==TemplateId.Team1Badge?1:id==TemplateId.Team2Badge?2:id==TemplateId.Team3Badge?3:id==TemplateId.Team4Badge?4:0;if(badgeRow>0)return PresentRows.Contains(badgeRow)?ImageMatchResult.FoundAt(8,300+((badgeRow-1)*70),20,20):ImageMatchResult.NotFound();int center=r.Value.Y+(r.Value.Height/2);int readyRow=((center-310+35)/70)+1;bool found=id==TemplateId.WorldMapTeamReadyAnchor&&ReadyRows.Contains(readyRow);return found?ImageMatchResult.FoundAt(70,r.Value.Y+10,20,20):ImageMatchResult.NotFound();}}
