...
internal static class Program
{
    ...
    static void TeamSelectionClearsStaleTerritoryColor(){string code=File.ReadAllText(Path.Combine(Environment.CurrentDirectory,"ADB","UI","DeviceDiagnosticWindow.xaml.cs"));int start=code.IndexOf("progress.CurrentStep == OneShotFarmStep.OpenTeamSelection",StringComparison.Ordinal);Is(start>=0&&code.Substring(start,Math.Min(350,code.Length-start)).Contains("TerritoryColor = string.Empty"),"stale territory color is not cleared for team selection");}
