...
internal static class Program
{
    ...
    static void AvailabilityUsesRosterRoi(){var f=new AvailabilityFixture();f.Matcher.ReadyRows.Add(4);var r=f.Service.CheckAsync("LDPlayer",default(CancellationToken)).GetAwaiter().GetResult();Is(r.Success&&r.AnyReadyTeam,"ready not found");Eq(1,f.Client.Captures,"captures");Eq(8,f.Matcher.Regions.Count,"badge and ready row scans");for(int i=0;i<4;i++){foreach(var region in f.Matcher.Regions.Skip(i*2).Take(2)){Is(region.Value.X==0&&region.Value.Y==290+(i*60)&&region.Value.Width==150&&region.Value.Height==60,"wrong row ROI "+i);}}}
    ...
    static void OneShotUiIsFocused(){string xaml=File.ReadAllText(Path.Combine(Environment.CurrentDirectory,"ADB","UI","DeviceDiagnosticWindow.xaml"));foreach(string hidden in new[]{"Tap Test","Swipe Test","Check Device","Launch Game","Capture Screenshot","Detect Current State","Ensure World Map","Open Search Panel","Configure Search","Search Iron","Verify Resource Popup","Open Team Selection","Select Farm Team","Dispatch Selected Team","Back Test","Text=\"Package name\"","Text=\"State name\"","Text=\"Note\""})Is(!xaml.Contains(hidden),"manual control remains: "+hidden);Is(xaml.Contains("RunOneShotFarmButton")&&xaml.Contains("StopOneShotFarmButton"),"farm controls missing");Is(xaml.Contains("Header=\"Thiết lập nâng cao\"")&&xaml.Contains("FarmProgressItemsControl")&&xaml.Contains("Chi tiết lần chạy gần nhất"),"compact operations layout missing");}
    ...
    sealed class AvailabilityFixture{public FakeNav Nav=new FakeNav();public AvailabilityDetector Detector=new AvailabilityDetector();public AvailabilityClient Client=new AvailabilityClient();public FakeRegistry Registry=new FakeRegistry();public AvailabilityMatcher Matcher=new AvailabilityMatcher();public WorldMapTeamAvailabilityService Service;public AvailabilityFixture(){Service=new WorldMapTeamAvailabilityService(Nav,Detector,Client,Registry,Matcher,new RecordingLock(),new WorldMapTeamAvailabilityOptions(new ImageRegion(0,290,150,240)),new Log());}}
