...
internal static class Program
{
    ...
    static void FarmUiPreservesScannedTeamStatuses(){string root=Path.Combine(Environment.CurrentDirectory,"ADB");string ui=File.ReadAllText(Path.Combine(root,"UI","DeviceDiagnosticWindow.xaml.cs"));string runner=File.ReadAllText(Path.Combine(root,"Infrastructure","Workflows","MultiDeviceOneShotFarmRunner.cs"));string readyGate=File.ReadAllText(Path.Combine(root,"Infrastructure","Workflows","ReadyTeamOneShotFarmWorkflow.cs"));Is(ui.Contains("IReadOnlyList<TeamNumber> busy = progress.BusyTeams")&&ui.Contains("IReadOnlyList<TeamNumber> locked = progress.LockedTeams")&&ui.Contains("string status = isLocked ? \"Khóa\"")&&ui.Contains(": isBusy ? \"Bận\""),"waiting card invents unscanned team states");Is(ui.Contains("FormatTeamFingerprint(farm?.BusyTeams)")&&ui.Contains("FormatTeamFingerprint(farm?.LockedTeams)"),"coalesced update can lose a changed scanned team state");Is(runner.Contains("CreateAvailabilityProgress(request, availability, stage, status)")&&runner.Contains("BusyTeams = (availability?.BusyTeams")&&runner.Contains("LockedTeams = (availability?.LockedTeams"),"preflight scan does not publish its roster status");Is(readyGate.Contains("waiting.BusyTeams = check.BusyTeams")&&readyGate.Contains("waiting.LockedTeams = check.LockedTeams"),"waiting result drops its roster status");}
