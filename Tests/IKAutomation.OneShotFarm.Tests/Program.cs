...
internal static class Program
{
    ...
    static void AvailabilityUsesRosterRoi(){var f=new AvailabilityFixture();f.Matcher.ReadyRows.Add(4);var r=f.Service.CheckAsync("LDPlayer",default(CancellationToken)).GetAwaiter().GetResult();Is(r.Success&&r.AnyReadyTeam,"ready not found");Eq(3,f.Client.Captures,"captures");Eq(6,f.Matcher.FrameBatches,"one existence and one status batch per frame");Eq(60,f.Matcher.Regions.Count,"existence and status scans");for(int frame=0;frame<3;frame++){int start=frame*20;for(int i=0;i<4;i++){var badgeRegion=f.Matcher.Regions[start+i].Value;Eq(290+(i*70),badgeRegion.Y,"wrong badge ROI");Eq(70,badgeRegion.Height,"wrong badge ROI height");}for(int i=0;i<4;i++){var region=f.Matcher.Regions[start+4+(i*4)].Value;Eq(290+(i*70),region.Y,"wrong row top "+i);Eq(70,region.Height,"wrong row height "+i);Eq(150,region.Width,"wrong row width "+i);}}}
    static void AvailabilityDefaultRowHeight(){var options=new WorldMapTeamAvailabilityOptions(new ImageRegion(0,290,150,280));Eq(70,options.TeamRowHeight,"default row height");var layout=WorldMapTeamRosterLayoutResolver.Resolve(1280,720,options);Is(layout!=null&&layout.Rows.Count==4,"row layout");Eq(570,layout.Rows[3].Y+layout.Rows[3].Height,"final row bounds");}
