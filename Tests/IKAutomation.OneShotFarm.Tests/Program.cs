...
internal static class Program
{
    ...
    static int Main()
    {
        ...
        Run("WorldMap readiness treats an unmatched one-team roster as busy", AvailabilityInfersBusySingleTeam);
    ...
    static void AvailabilityInfersBusySingleTeam(){var f=new AvailabilityFixture();f.Matcher.PresentRows.Clear();var r=f.Service.CheckAsync("LDPlayer",default(CancellationToken)).GetAwaiter().GetResult();Is(r.Success&&!r.AnyReadyTeam,"busy one-team roster should enter the readiness wait");Is(r.AvailableTeams.SequenceEqual(new[]{TeamNumber.Team1}),"single-team roster was not inferred");Eq(0,r.ReadyTeams.Count,"busy team was reported ready");Eq(0,f.Client.Inputs,"input");}
