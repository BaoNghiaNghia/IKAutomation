...
internal static class Program
{
    ...
    static void AvailabilityUsesRosterRoi(){var f=new AvailabilityFixture();f.Matcher.ReadyRows.Add(4);var r=f.Service.CheckAsync("LDPlayer",default(CancellationToken)).GetAwaiter().GetResult();Is(r.Success&&r.AnyReadyTeam,"ready not found");Eq(2,f.Client.Captures,"captures");Eq(16,f.Matcher.Regions.Count,"badge and ready scans");for(int frame=0;frame<2;frame++){for(int i=0;i<4;i++){var badgeRegion=f.Matcher.Regions[(frame*8)+i].Value;Eq(290,badgeRegion.Y,"wrong badge ROI");Eq(280,badgeRegion.Height,"wrong badge ROI height");}for(int i=0;i<4;i++){var region=f.Matcher.Regions[(frame*8)+4+i].Value;Eq(298+(i*52),region.Y,"wrong row top "+i);Eq(52,region.Height,"wrong row height "+i);Eq(150,region.Width,"wrong row width "+i);}}}
