...
internal static class Program
{
    ...
    static OneShotFarmWorkflow PlanWorkflow(H h,FakePlan plan)=>new OneShotFarmWorkflow(h.Nav,new FakeFallback(h.Config,h.Search),h.Popup,h.Open,h.Select,h.Dispatch,h.Detector,h.Lock,new OneShotFarmWorkflowOptions(true,true,"Diagnostics/OneShotFarm"),h.Diag,new Log(),new ResourceFarmFallbackOptions(),plan);
    ...
    sealed class FakePlan:IResourceFarmFallbackService{public int Calls;public Action DuringRun;public Task<ResourceFarmFallbackResult> RunAsync(string d,OneShotFarmRequest r,GameState s,CancellationToken t){t.ThrowIfCancellationRequested();Calls++;DuringRun?.Invoke();return Task.FromResult(new ResourceFarmFallbackResult{Outcome=ResourceFarmFallbackOutcome.MarchStarted,Success=true,RequestedResources=r.ResourcePriority,AttemptedResources=new[]{ResourceType.Iron},StorageFullResources=new ResourceType[0],LevelsExhaustedResources=new ResourceType[0],LocatedResource=ResourceType.Iron,LocatedLevel=7,DispatchedResource=ResourceType.Iron,DispatchedTeam=TeamNumber.Team4,Attempts=new ResourceFarmAttemptResult[0],InitialState=s,FinalState=GameState.WorldMap,Message="march"});}}
