...
internal static class Program
{
    ...
    static void PreferenceSafeDefaults(){var p=new FarmUiPreferences();int count=(p.Iron?1:0)+(p.Stone?1:0)+(p.Wood?1:0)+(p.Food?1:0);Is(count>=2,"resource count");Eq(4,p.ReadyCheckIntervalMinutes,"default ready check interval");Eq(240000,FarmUiPreferencesMapper.CreateRequest(p,new OneShotFarmRequest()).ReadyTeamOptions.CheckIntervalMs,"default ready check delay");Is(FarmUiPreferencesMapper.Validate(p).IsValid,"safe defaults invalid");}
