...
internal static class Program
{
    ...
    static int Main()
    {
        ...
        Run("Panel failure stops before configure", PanelFail); Run("Configure failure stops before search", ConfigureFail);
        ...
        Run("ResourceNotFound has no level fallback", NoLevelFallback); Run("Success requires MarchStartedVerified", VerifiedRequired);
    ...
    static void StepOrder(){var h=new H();var s=Go(h).Steps.Select(x=>x.Step).ToArray();var expected=new[]{OneShotFarmStep.Preflight,OneShotFarmStep.EnsureWorldMap,OneShotFarmStep.OpenSearchPanel,OneShotFarmStep.ConfigureSearch,OneShotFarmStep.ExecuteSearch,OneShotFarmStep.VerifyResourcePopup,OneShotFarmStep.OpenTeamSelection,OneShotFarmStep.SelectTeam,OneShotFarmStep.DispatchTeam,OneShotFarmStep.FinalVerification,OneShotFarmStep.Completed};Is(expected.SequenceEqual(s),"order");}
    ...
    static void NoLevelFallback(){var h=new H();h.Search.Outcome=ResourceSearchOutcome.ResourceNotFound;Go(h);Eq(1,h.Config.Calls,"fallback config");Eq(7,h.Config.Last.TargetLevel,"level");}
    ...
    sealed class H
    {
        ...
        public H(){Workflow=new OneShotFarmWorkflow(Nav,Config,Search,Popup,Open,Select,Dispatch,Detector,Lock,new ResourceSearchConfigurationOptions(1,1,1,1,7,8,1),new OneShotFarmWorkflowOptions(true,true,"Diagnostics/OneShotFarm"),Diag,new Log());}
    ...
    sealed class FakeNav:IWorldMapNavigationService{public int EnsureCalls,PanelCalls;public bool EnsureSuccess=true,PanelSuccess=true;public Action AfterEnsure;public Task<NavigationResult> EnsureWorldMapAsync(string d,CancellationToken t){EnsureCalls++;AfterEnsure?.Invoke();return Task.FromResult(new NavigationResult{Success=EnsureSuccess,FinalState=GameState.WorldMap,Message="world"});}public Task<NavigationResult> OpenResourceSearchPanelAsync(string d,CancellationToken t){PanelCalls++;return Task.FromResult(new NavigationResult{Success=PanelSuccess,FinalState=PanelSuccess?GameState.ResourceSearchPanel:GameState.WorldMap,Message="panel",FinalEvidence=PanelSuccess?new[]{new GameDetectionEvidence{TemplateId=TemplateId.ResourceSearchPanelAnchor,Found=true}}:new GameDetectionEvidence[0]});}}
    ...
    sealed class FakeSearch:IResourceSearchExecutionService{public int Calls;public bool Success=true;public ResourceSearchOutcome Outcome=ResourceSearchOutcome.ResourceLocated;public ResourceSearchExecutionRequest Last;public Task<ResourceSearchExecutionResult> ExecuteAsync(string d,ResourceSearchExecutionRequest r,CancellationToken t){Calls++;Last=r;return Task.FromResult(new ResourceSearchExecutionResult{Success=Success&&Outcome==ResourceSearchOutcome.ResourceLocated,Outcome=Outcome,FinalState=Outcome==ResourceSearchOutcome.ResourceLocated?GameState.ResourcePopup:GameState.ResourceSearchPanel,Message="search"});}}
    sealed class FakePopup:IResourcePopupVerificationService{public int Calls;public bool Ready=true;public Task<ResourcePopupVerificationResult> VerifyAsync(string d,CancellationToken t){Calls++;return Task.FromResult(new ResourcePopupVerificationResult{Success=Ready,Outcome=Ready?ResourcePopupOutcome.ResourcePopupReady:ResourcePopupOutcome.ResourcePopupDetectedButNotReady,PopupAnchorVerified=Ready,IronResourceVerified=Ready,GatherButtonVerified=Ready,FinalState=GameState.ResourcePopup,Message="popup"});}}
    ...
    sealed class FakeDispatch:IDispatchSelectedTeamService{public int Calls;public bool Verified=true;public DispatchMarchOutcome Outcome=DispatchMarchOutcome.MarchStarted;public DispatchMarchRequest Last;public Task<DispatchMarchResult> DispatchAsync(string d,DispatchMarchRequest r,CancellationToken t){Calls++;Last=r;return Task.FromResult(new DispatchMarchResult{Success=Verified,Outcome=Outcome,MarchStartedVerified=Verified,DispatchedTeam=r.ExpectedTeam,FinalState=GameState.WorldMap,Message="dispatch"});}}
