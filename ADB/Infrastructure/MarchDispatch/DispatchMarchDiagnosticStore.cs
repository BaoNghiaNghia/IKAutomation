...
{
    public sealed class DispatchMarchDiagnosticStore : IDispatchMarchDiagnosticStore
    {
        private readonly string root;
        public DispatchMarchDiagnosticStore(string rootDirectory)
        {
        ...
        public async Task<string> SaveAsync(string deviceName, DispatchMarchOutcome outcome,
            byte[] screenshotPng, CancellationToken cancellationToken)
        {
            ...
            string directory = Path.Combine(root, ScreenshotPathPolicy.SanitizeDeviceName(deviceName), now.ToString("yyyy-MM-dd"));
            ...
            throw new IOException("Could not create a unique March Dispatch diagnostic screenshot.");
        }
    }
}
